...
class ColumnMappingDialog(simpledialog.Dialog):
    ...
    * optional extra filters in the form
    ...
    def body(self, master):
        ...
        (ttk.Label(master, text='X-coordinate column; e.g. CURRENT_FIX_X_INTEREST_AREA:')
         .grid(row=0, column=0, sticky='w', pady=2))
        self.pixel_x_entry = ttk.Entry(master, width=30)
        self.pixel_x_entry.grid(row=0, column=1, pady=2)

        (ttk.Label(master, text='Y-coordinate column; e.g. CURRENT_FIX_Y_INTEREST_AREA:')
         .grid(row=1, column=0, sticky='w', pady=2))
        self.pixel_y_entry = ttk.Entry(master, width=30)
        self.pixel_y_entry.grid(row=1, column=1, pady=2)

        (ttk.Label(master, text='Interest area label column; e.g. CURRENT_FIX_INTEREST_AREA_LABEL:')
    ...
    def apply(self):
        ...
        if not interest_area_label:
            messagebox.showerror("Error", "Interest area label column name is required.")
            return None
