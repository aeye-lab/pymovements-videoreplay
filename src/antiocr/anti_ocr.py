...
class AntiOCR:
    ...
    def generate_from_csv(self, csv_path: str, page_name: str, session: str, output_path: str):
        ...
        """
        try:
            csv_files = [f for f in Path(csv_path).glob('*.csv') if 'fixfinal' in f.name]
            if not csv_files:
            ...
            column_mapping = {
                ...
                'CURRENT_FIX_INTEREST_AREA_LABEL': 'word',  # Label which should be displayed at that coordinate
            ...
            df = df[(df['page_name'] == page_name) & (df['recording_session'] == session)]
        ...
        image = np.ones((self.frame_height, self.frame_width, 3), dtype=np.uint8) * 255
        ...
        if not any(str(output_path).lower().endswith(ext) for ext in valid_extensions):
            print("ERROR: Output file must have a valid image extension (e.g., .png or .jpg)")
            return
